...
17. **Admin Permissions Fixes & UI Enhancements**: (✅ Completed) Replaced restrictive `role !== "ADMIN"` hardcoded checks with dynamic role-based functions (e.g. `canManageStore()`) across all admin routes (Reviews, QA, Flash Sales, etc.). Replaced generic `window.confirm` dialogues with premium custom Glassmorphic Modals for actions like closing tickets and changing user roles.
